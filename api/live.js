// Serverless (Vercel) — jala TODAS las citas de Be Well Esthetic desde GoHighLevel.
...
async function pull() {
  ...
  const citas = ev.map((e) => ({
    fecha: (e.startTime || "").slice(0, 10),
    hora: (e.startTime || "").slice(11, 16),
    cliente: (e.title || e.contactName || "Cliente").trim(),
    tipo: (e._cal || "").replace(/^Be Well Esthetic\s*[-·]?\s*/i, "").trim() || "Cita",
    estado: e.appointmentStatus || "new",
  }));

  return { total, cerradas, conv, by, citas, ts: Date.now() };
}
